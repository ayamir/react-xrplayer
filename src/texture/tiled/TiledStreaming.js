...
class TiledStreaming {
  ...
  onTileButtonClick = (i) => {
    ...
    if (this.selected[i]) {
      ...
      buffer_info.innerHTML =
        "buffer:" + this.enhanceDash[i].getBufferLength("video");
      level_info.innerHTML =
        "level:" + this.enhanceDash[i].getQualityFor("video");
      throughput.innerHTML =
        "throughput:" + this.enhanceDash[i].getAverageThroughput("video");
      let levelList = "bitrates:";
      let bitrateList = this.enhanceDash[i]
        .getActiveStream()
        .getBitrateListFor("video");
  ...
  loadTile = (id, level) => {
    ...
    if (this.videoMediaAsyns[id] === null) {
      this.videoMediaAsyns[id] = new MCorp.mediaSync(
        this.enhanceVideos[id],
        this.timingAsynSrc
      );
    } else {
...
>>>>>>> d4ef3f3114a34f50dd5a3127e967a3619313a278
  ...
  initVideoNode = (videoInstance, width, height) => {
    ...
    videoInstance.setAttribute("x5-video-player-fullscreen", false);
